    ...
    axios
      ...
      .catch(response => {
        if (response instanceof Error) {
          ...
          console.log(
            'Error:',
            'That request cannot be processed. Please check the address.'
          );
        } else {
  ...
  render() {
    const redirect = this.state.redirect ? (
      <Redirect
        push
        to={{
          // push keyword required for back clicking
      ...
      <Container>
        ...
        <Row style={{ marginTop: '5rem' }}>
          <Col xs="12" sm="12" md={{ size: 8, offset: 2 }}>
            <AddressSearch
              adressHandler={this.updateAddress}
              addressValue={this.state.address}
            />
            <Button
              style={{ backgroundColor: '#fc41a1', color: '#fff' }}
